...
import ImagePicker from 'react-native-image-crop-picker';
import { useSelector } from 'react-redux';
import { hitAddBankAccount, hiteEditBankAccount } from '../../config/api/api';
import Loading from '../../components/Loading/Loading';
import { errorToast, successToast } from '../../common/CommonFunction';
import HeaderBackButton from '../../components/HeaderBackButton/HeaderBackButton';
import ImagePickerComponent from '../../components/ImagePickerComponent/ImagePicker';
import { responsiveFontSize, responsiveHeight } from '../../common/metrices';
// import { successToast Assuming you have a utility function for showing toasts
const UpdateBankDetailsScreen = ({ navigation, route }) => {
  ...
  const handleAddAccount = async () => {
    try {
      ...
      const payload = isEdit
        ? {
            id: bankData?.id, // required by edit API
            account_no: accountNumber,
            ifsc_code: ifscCode,
            branch_name: '-', // or bankData?.branch_name if available
          }
        : {
            partner_id: partnerId,
            ifsc_code: ifscCode,
      ...
      const response = isEdit
        ? await hiteEditBankAccount(payload)
        : await hitAddBankAccount(payload);
      if (response?.success) {
        successToast(
          'Success!',
          isEdit ? 'Bank details updated successfully!' : 'Bank details added successfully!'
        );
        setAccountNumber('');
        setIfscCode('');
        setBankDocument(null);
      ...
      console.error(error);
      errorToast(error.error, 'An error occurred while saving bank details!');
    } finally {
      setLoading(false);
    }
  };
  ...
  React.useEffect(() => {
    if (bankData) {
      ...
      if (bankData?.document_pic?.[0]?.img_src) {
        setBankDocument({
          uri: `data:image/png;base64,${bankData.document_pic[0].img_src}`,
          base64: bankData.document_pic[0].img_src,
        });
      }
    }
  }, [bankData]);
  return (
    <>
      ...
      /> */}
      <HeaderBackButton
  headerText={bankData ? 'Update Bank Details' : 'Manage Bank Account'}
...
/>
  ...
  {bankData ? 'Update Details' : 'Proceed'}
</Text>
          </TouchableOpacity>
          <View style={{ alignItems: 'center', marginTop: 10 }}>
          </View>
