...
import { errorToast, getItem, setItem, successToast } from '../../common/CommonFunction'
import { useDispatch, useSelector } from 'react-redux'
import { hitGetDriverDetails, hitGetUserOrderStatsApi, hitGetWalletBalanceApi, hitVerifyEmail } from '../../config/api/api'
import { PROFILE_BASE_URL } from '../../common/Constant'
// import { fetchProfileThunk, setUserStatusIdle } from '../../redux/HitApis/HitApisSlice'
import { Button, Dialog, Portal } from 'react-native-paper'
import BottomNav from '../../../navigation/BottomNav'
import AppImages from '../../common/AppImages'
import { setlivetripmenu, setloginuserdetails, setOrderData, setwalletBalance } from '../../redux/HitApis/HitApiSlice'
...
const AccountScreen = () => {
    ...
    const profileData = useSelector((state) => state?.parsal_store || {});
    const customerId = useSelector((state) => state.parsal_store?.profileData?.user?.id || {});
    const nameFromRedux = useSelector((state) => state.parsal_store?.profileData?.user?.cust_name || "Hello");
    ...
    useEffect(() => {
        if (profileData && customerId) {
            // setName(profileData?.profileData?.user?.cust_name || '');
            // setEmail(profileData?.profileData?.user?.email || '');
            // setProfilePic(
            //     profileData?.profileData?.user?.profile_pic == '-' || !profileData?.profileData?.user?.profile_pic
            //         ? '-'
            //         : PROFILE_BASE_URL + customerId + '/' + profileData?.profileData?.user?.profile_pic
            // );
        }
    }, [profileData, customerId, isFocused]);
    const [orderStats, setorderstats] = useState([])
    const fetchData = async () => {
        const getCustomerId = await getItem('customerId');
        const request = { id: getCustomerId };
        dispatch(fetchProfileThunk(request));
        hitGetUserOrderStatsApi(request).then((res) => {
    ...
    console.log('parse_data?.payload?.owner_type',store_data.wallet_balance?.new_wallet_balance);
    ...
    return (
        <>
            <View style={{ flex: 1, backgroundColor: "#F5F6F7" }}>
                <SafeAreaView style={{ backgroundColor: "#F5F6F7", flex: 1, marginHorizontal: 16 }} >
                    <ScrollView showsVerticalScrollIndicator={false}>

                        <View style={styles.profileSection}>
                            <Text style={styles.profileTextStyle}>{`Profile`}</Text>
                            <TouchableOpacity onPress={() => navigation.navigate('EditProfileScreen')} activeOpacity={0.8}>
                        ...
                        <View style={{ flex: 15, backgroundColor: '#F5F6F7' }}>
                            <View style={styles.userDetailSection}>
                                <View style={styles.udSection1}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        {/* <Image
                                        key={profilePic}
                                        source={profilePic == '-' ? AppImages.dumm : { uri: profilePic + '?=' + new Date() }}
                                        resizeMode='contain' style={styles.man}
                                    /> */}
                                        <Image
                                            source={{
                                        ...
                                        <View style={styles.userTextContainer}>
                                            <Text style={styles.name}>{user_details?.driver_name?.toLocaleUpperCase() ||
                                                user_details?.partner_name?.toLocaleUpperCase()}</Text>
                                            <Text style={styles.email}> {user_details?.phone}</Text>
                                            <TouchableOpacity onPress={() => {
                                                handleVerifyEmail()
                                            }}>
                                                <Text style={styles.VerifyEmail}>{'Verify Email ID'}</Text>
                                            </TouchableOpacity>
                                        </View>
                                    </View>
                                </View>
                                <View style={styles.udSection2}>
                                    ...
                                    <View style={styles.dataContainer}>
                                        <Data number={orderStats?.totalOrderCount || 0} dataName={'Shipments'} />
                                        <Data number={orderStats?.totalOrderCount || 0} dataName={'Transactions'} />
                                        <Data number={'₹' + orderStats?.totalPaidAmount || 0} dataName={'Spendings'} />
                                    </View>
                                </View>
                            </View>
                        ...
                        <Portal>
                            <Dialog visible={visible} onDismiss={hideDialog}>
                                ...
                                <Dialog.Content>
                                    <Text variant="bodyMedium" style={{ color: Colors.black, fontSize: responsiveFontSize(16) }}>Are you sure you want to logout?</Text>
                                </Dialog.Content>
                                <Dialog.Actions>
                                    <Button onPress={hideDialog} textColor={Colors.brandBlue}>Cancel</Button>
                                    ...
                                    }} textColor={Colors.brandBlue}>Yes</Button>
                                </Dialog.Actions>
                            </Dialog>
                        </Portal>
                    </ScrollView>
