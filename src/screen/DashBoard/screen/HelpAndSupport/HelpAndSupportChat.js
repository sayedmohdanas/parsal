...
//   const [visibleDate, setVisibleDate] = useState(''); // Added state to store the current visible date label
...
//       return 'Yesterday';
//     } else if (messageDate.toDateString() === dayBeforeYesterday.toDateString()) {
...
//     const visibleIndex = Math.floor(event.nativeEvent.contentOffset.y / 60); // Adjust scroll offset for message height
//     if (messages[visibleIndex]) {
//       setVisibleDate(formatDateLabel(messages[visibleIndex].date_time)); // Set the visible date label based on the message's date
//     }
//   };
...
//         by_whom: 2,
//         status: 1,
//         date_time: new Date().toISOString(), // Adding current date-time for display purposes
...
//     const maxWidth = windowWidth * 0.7; // Maximum width is 70% of the screen width
...
//           {
//             maxWidth: maxWidth, // Restrict the message width to 70% of screen width
//             minWidth: 50,       // Minimum width to ensure short messages look good
//           },
//         ]}>
...
//     padding: 10,
//     borderRadius: responsiveHeight(10),
//     // width: responsiveWidth(Dimensions.get('window').width) / 2.5,
//     // width: 200,
//   },
//   sentMessage: {
//     alignSelf: 'flex-end',
...
//   dateLabelContainer: {
//     position: 'absolute',
//     top: responsiveHeight(5), // Adjust the top offset as needed
//     alignSelf: 'center',
...
const HelpAndSupportChat = ({ route }) => {
  ...
  const handleSend = async () => {
    if (messageText.trim()) {
      const newMessage = {
        ...
        date_time: new Date(new Date().setDate(new Date().getDate() - 1)).toISOString(), // Adding yesterday's date-time
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior="padding">
      ...
      <FlatList
        refreshControl={<RefreshControl onRefresh={fetchMessages} refreshing={refreshing} />}
        data={messages}
        keyExtractor={(item, index) => index.toString()}
        renderItem={renderItem}
        onScroll={handleScroll}
...
const styles = StyleSheet.create({
  ...
  quickResponsesContainer: { padding: 10, backgroundColor: Colors.white, alignItems: 'center', flexDirection: 'row', flexWrap: 'wrap' },
  messageList: { paddingHorizontal: responsiveWidth(10), paddingTop: responsiveHeight(10) },
  messageContainer: { marginVertical: responsiveHeight(5), padding: 10, borderRadius: responsiveHeight(10) },
  ...
  timestamp: { fontSize: responsiveFontSize(10), color: Colors.grey, marginTop: 5 },
  inputContainer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 10 },
  input: { flex: 1, height: 40, borderRadius: 20, paddingHorizontal: 15, backgroundColor: '#f1f1f1' },
  sendButton: { paddingHorizontal: 15, paddingVertical: 10, backgroundColor: Colors.primary, borderRadius: 20 },
