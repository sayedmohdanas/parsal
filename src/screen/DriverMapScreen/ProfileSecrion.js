...
const ProfileSection = () => {
  ...
  const handleAction = type => {
    ...
    if (orderData?.custMobile) {
      const url =
        type === 'call'
          ? `tel:${orderData.custMobile}`
          : `sms:${orderData.custMobile}`;

      // Use Linking to open the appropriate app
      Linking.openURL(url).catch(err =>
        console.error(
          `Error opening ${type === 'call' ? 'dialer' : 'messaging app'}: `,
          err,
        ),
      );
    } else {
      ...
      <View style={styles.profileContainer}>
        ...
        <View style={{width: '100%', backgroundColor: Colors.white}}>
          <View style={{flex: 1, flexDirection: 'row', margin: 4}}>
            ...
            <View
              style={{flex: 1, marginHorizontal: 6, justifyContent: 'center'}}>
              <Text
